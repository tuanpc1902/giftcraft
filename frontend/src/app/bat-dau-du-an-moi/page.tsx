...
function StartProjectInner() {
  ...
  if (done) {
    return (
      <div className="max-w-xl mx-auto px-4 py-24 text-center">
        ...
        <p className="text-gray-500 mb-2">Đội ngũ tư vấn sẽ liên hệ trong <strong>24h làm việc</strong>.</p>
        {orderRef && <p className="text-xs text-gray-400 mb-8 font-mono">Mã yêu cầu: #{orderRef}</p>}
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-12">
      <h1 className="text-2xl font-bold text-gray-900 mb-2">Bắt đầu dự án quà tặng</h1>
      <p className="text-gray-500 text-sm mb-8">Điền thông tin — nhận báo giá trong 24h, không cần gọi điện.</p>
      ...
      <div className="flex items-center gap-1 mb-10 overflow-x-auto">
        {steps.map((label, i) => (
          <div key={i} className="flex items-center gap-1 flex-shrink-0">
            ...
            <span className={`hidden sm:block text-xs whitespace-nowrap ${state.step === i + 1 ? "text-gray-700 font-medium" : "text-gray-400"}`}>{label}</span>
            {i < 4 && <div className="w-4 sm:w-6 h-px bg-gray-200" />}
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6">
        ...
        {state.step === 1 && (
          <>
            <h2 className="font-bold text-gray-900 mb-5">Loại dự án của bạn?</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {PROJECT_TYPES.map(pt => (
                <button key={pt.value} onClick={() => set({ projectType: pt.value })}
                  className={`flex items-center gap-3 p-4 rounded-2xl border-2 transition-colors text-left ${state.projectType === pt.value ? "border-gray-900 bg-gray-50" : "border-gray-100 hover:border-gray-200"}`}>
                  <span className="text-2xl">{pt.icon}</span>
                  <span className="font-medium text-gray-800">{pt.label}</span>
                </button>
              ))}
            </div>
        ...
        {state.step === 2 && (
          <>
            <h2 className="font-bold text-gray-900 mb-2">Sản phẩm (tùy chọn)</h2>
            <p className="text-sm text-gray-500 mb-5">Đã biết sản phẩm muốn đặt? Nhập tên hoặc bỏ qua.</p>
            <input className="input-field" placeholder="Ví dụ: BÁCH VỊ VẠN HỶ, Hương Quê..." value={state.occasion}
              onChange={e => set({ occasion: e.target.value })} />
            <p className="text-xs text-gray-400 mt-3">Hoặc để chúng tôi tư vấn sản phẩm phù hợp.</p>
            <div className="flex justify-between mt-6">
              <button onClick={back} className="text-gray-500 hover:text-gray-700 font-medium">← Quay lại</button>
        ...
        {state.step === 3 && (
          <>
            <h2 className="font-bold text-gray-900 mb-5">Số lượng &amp; ngân sách</h2>
            <div className="space-y-5">
              <div>
                <label className="text-sm font-medium text-gray-700 block mb-2">Số lượng sản phẩm</label>
                <input type="number" min="1" className="input-field" value={state.quantity}
                  onChange={e => set({ quantity: parseInt(e.target.value) || 1 })} />
                {tierLabel && (
                  <p className="mt-2 text-sm text-amber-600 font-medium">🎉 {state.quantity} sản phẩm → {tierLabel}</p>
                )}
                {state.quantity < 20 && (
                  <p className="mt-2 text-sm text-gray-400">Giá B2B áp dụng từ 20 sản phẩm</p>
                )}
              </div>
              <div>
                <label className="text-sm font-medium text-gray-700 block mb-2">Ngân sách dự kiến (VNĐ/bộ)</label>
              ...
              <div>
                <label className="text-sm font-medium text-gray-700 block mb-2">Deadline giao hàng</label>
        ...
        {state.step === 4 && (
          <>
            <h2 className="font-bold text-gray-900 mb-5">Yêu cầu tùy chỉnh</h2>
            <div className="space-y-2 mb-5">
              {CUSTOMIZATIONS.map(c => (
                <label key={c.key} className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-colors ${state.customizations.includes(c.key) ? "border-amber-400 bg-amber-50" : "border-gray-100"}`}>
            ...
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-2">Mô tả thêm (tùy chọn)</label>
        ...
        {state.step === 5 && (
          <>
            <h2 className="font-bold text-gray-900 mb-5">Thông tin liên hệ</h2>
            ...
            <div className="bg-gray-50 rounded-xl p-4 mt-5 text-sm text-gray-600 space-y-1">
              <p className="font-semibold text-gray-800 mb-2">Tóm tắt yêu cầu:</p>
