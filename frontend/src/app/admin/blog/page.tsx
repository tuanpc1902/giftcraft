              ...
              {posts.map(post => (
                <tr key={post.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right flex items-center justify-end gap-2">
                    <button onClick={() => openEdit(post)} className="text-xs text-amber-600 hover:text-amber-700 font-medium">Sửa</button>
                    <button onClick={() => handleDelete(post.id)} className="text-xs text-red-500 hover:text-red-700 font-medium">Xóa</button>
                  </td>
                </tr>
              ))}
              {posts.length === 0 && (
                <tr><td colSpan={5} className="text-center py-12 text-gray-400">Chưa có bài viết nào.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-start justify-center overflow-y-auto py-8 px-4">
          <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl">
            <div className="p-6 border-b border-gray-100 flex items-center justify-between">
              <h2 className="font-bold text-gray-900">{selected ? "Chỉnh sửa bài viết" : "Bài viết mới"}</h2>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Slug (tự động nếu bỏ trống)</label>
